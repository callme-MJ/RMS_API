import { Controller,Req,HttpStatus,Delete,Param,ParseIntPipe, Get,Body, Post, Res, UsePipes, ValidationPipe } from '@nestjs/common';
import { Request } from 'express';
import { candidateDto } from 'src/colleges/dtos/canditate.dto';
...
@Controller('colleges')
    ...
    @Get("candidates/:id")
    async findCandidateById(@Res() response, @Param("id",ParseIntPipe) id:number ){
    ...
    async querybuilder(@Req() req:Request){
        ...
        if (req.query.search) {
            builder.where("candidates.id LIKE :search OR candidates.name LIKE :search OR candidates.ad_no LIKE :search", {search: `%${req.query.search}%`})
        }
